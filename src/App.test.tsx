...
const service: IDataService = {
  getJobsWithSearchTerm: (searchTerm: string) => {
    return Promise.resolve(data.filter(({ name }) => name.toLowerCase().includes(searchTerm)))
  },
  getJobs: () => { return Promise.resolve(data) }
}
...
describe('Skedulo tech test', () => {
  ...
  test('can search for data', async () => {
    ...
    await waitFor(async () => {
      expect(screen.getByText((node) => node.includes('Build a fence'))).toBeInTheDocument()
      expect(screen.getByText((node) => node.includes('Build a shed'))).toBeInTheDocument()
    })
  })
  ...
  test('clears results when input clears', async () => {
    ...
    await waitFor(async () => {
      expect(screen.queryByText((node) => node.includes('Build a fence'))).not.toBeInTheDocument()
      expect(screen.queryByText((node) => node.includes('Build a shed'))).not.toBeInTheDocument()
    })
  })
})